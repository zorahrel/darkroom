...
export const COMPOSITION = {
  ...
  recompose:
    REFRAME_FREEDOM +
    "This must be a photograph someone composed, not a rectangle cut out of a snapshot: choose a decisive point of view, work the diagonal, let the subject loom or sit off to one side against open space, and build depth with a real foreground, middle ground and background. Use the whole frame deliberately: the subject placed with intent, generous negative space where it earns tension, leading lines that carry the eye. Never a flat, dead-centre crop with the subject simply parked in the middle. Keep the subject unobstructed, and do not slice awkwardly through it — but only ever show the parts of it that were actually photographed: if the original never captured the base of a building, the foot of a statue or the ground it stands on, that part simply is not in this picture and must NOT be drawn in. A tighter frame is always allowed; conjuring the missing bottom of a subject is not",
