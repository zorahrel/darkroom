...
LA LUCE sul mio viso e' UNA sola, BLU-CIANO: un grande pannello morbidissimo con gelatina azzurra, quasi frontale e appena piu' alto degli occhi. E' l'unica luce che mi tocca il viso, quindi tutto il volto ha la STESSA tinta azzurro-ciano, OMOGENEA dalla fronte al mento e da una guancia all'altra: la pelle si legge fredda e azzurrata, come illuminata da un grande schermo blu in una stanza buia. Le parti piu' esposte (fronte, dorso del naso, zigomi) sono un ciano pallidissimo quasi bianco, i mezzitoni sono azzurro-lavanda, le ombre sfumano dolcemente in un blu profondo. Sotto la luce blu i miei lineamenti e il mio incarnato restano riconoscibili, come in un editoriale di moda illuminato solo con gelatine colorate. Tutta la foto vive nella stessa luce: viso, collo, felpa e fondo hanno la stessa dominante blu.
...
await withProject(PROGETTO, async () => {
  ...
  const varianti = [
    { nome: "blu-omogenea", prompt: PROMPT.replace(/ciano-verde/g, "ciano").replace(/verde-acqua\/ciano/g, "ciano").replace(/verde-ciano/g, "ciano") },
  ];
