...
await withProject(PROGETTO, async () => {
  ...
  const varianti = [
    { nome: "verbatim", prompt: PROMPT },
    { nome: "senza-verde", prompt: PROMPT.replace(/ciano-verde/g, "ciano").replace(/verde-acqua\/ciano/g, "ciano").replace(/verde-ciano/g, "ciano") },
  ];
  for (let g = 1; g <= giri; g++) {
    ...
    const job = enqueueJob(
      ...
      JSON.stringify({
        ...
        materia: MATERIA,
        cambiato: "blocco luce e fondo scritti da ChatGPT guardando la reference (deprompt_chatgpt.py): tre luci, key quasi neutra, fill blu-ciano basso che colora le ombre, luce turchese sul fondo. Key e fill ribaltati a destra/sinistra come da misura",
    ...
    console.log(`[tre-luci] job ${job.id}  ${v.nome}  verde:${(v.prompt.match(/verde/gi) || []).length}`);
  }
});
