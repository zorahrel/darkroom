...
withProject(pid, async () => {
  ...
  for (const photo of targets) {
    for (const recipe of recipes) {
      ...
      const job = enqueueJob(photo.id, recipe.prompt, JSON.stringify({ recipe: recipe.key }), "chatgpt", null, "edit", null, JSON.stringify(refs));
      ...
      if (res.status === "ok") {
        const ins = db().run(
          ...
          [photo.id, n, out, recipe.prompt, JSON.stringify({ recipe: recipe.key, backend: "codex-http" }), Date.now()],
        );
