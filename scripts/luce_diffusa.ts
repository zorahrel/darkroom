...
LA LUCE e' una vera luce da studio, grande e DIFFUSA: un ampio softbox con gelatina ciano chiaro, davanti a me e un po' piu' in alto, vicino al viso. E' LUCE, non un filtro colorato sopra la foto: costruisce il volume del viso. Le zone piu' vicine alla luce (fronte, dorso del naso, zigomi, arco di cupido, mento) sono LUMINOSISSIME, quasi bianche, con un velo ciano ghiaccio e una lucentezza morbida sulla pelle; da li' la luce scende con un passaggio lento e setoso verso guance e mascella, che prendono un azzurro piu' pieno, e verso ombre morbide blu notte sotto gli zigomi, sotto il mento e sul collo. La pelle resta pelle: grana, pori fini e volume si vedono attraverso la luce. Le lenti degli occhiali riflettono il softbox come un rettangolo chiaro. Il viso e' la parte piu' luminosa della foto; la felpa nera resta nera, e il blu del fondo e' quello del fondale, non un velo sul soggetto.
...
await withProject(PROGETTO, async () => {
  ...
  const varianti = [
    { nome: "luce-diffusa", prompt: PROMPT.replace(/ciano-verde/g, "ciano").replace(/verde-acqua\/ciano/g, "ciano").replace(/verde-ciano/g, "ciano") },
  ];
