...
LA MIA BOCCA resta esattamente quella della foto: labbra chiuse e rilassate, niente sorriso, niente denti. E' una bocca stretta: non allargarla e non ridisegnarla.

GLI OCCHIALI DA SOLE restano esattamente quelli della foto: stessa forma, proporzioni, spessore della montatura e curvatura.
...
- LUCE PRINCIPALE: un beauty dish grande e morbido con una gelatina CIANO GHIACCIO (icy cyan gel), il colore di una lampada fredda azzurra. E' una luce FORTE: fronte, dorso del naso e zigomi sono QUASI BIANCHI, bruciati, con appena una velatura ciano ghiaccio e un riflesso lucido. Il colore si vede nelle alte luci, che restano chiarissime: la pelle illuminata sembra bianca-azzurra, non colorata a meta' tono.
...
await withProject(PROGETTO, async () => {
  ...
  for (let g = 1; g <= giri; g++) {
    const job = enqueueJob(
      ...
      JSON.stringify(refs),
      JSON.stringify({
        recipe: "luce-gel-3-codex",
        materia: MATERIA,
        cambiato: "da v170: ciano ghiaccio sulle alte luci quasi bianche invece di verde-acqua sui mezzitoni; testa girata a destra e luce da destra, guancia sinistra in ombra, come la reference",
    ...
    console.log(`[gel3-codex] job ${job.id}  giro ${g}`);
  }
});
