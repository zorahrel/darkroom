...
LA LUCE sul mio viso e' un grande octabox con gelatina ciano-turchese pallido messo proprio dietro la macchina fotografica, appena sopra l'altezza dei miei occhi, che mi guarda dritto in faccia. Per questo tutta la parte frontale del viso riceve la STESSA quantita' di luce: fronte, guance, naso, labbra e mento sono luminosi allo stesso modo, e sulle parti piu' esposte la pelle sfiora il bianco. Non c'e' un lato in ombra e non c'e' un cerchio di luce al centro. Cambia solo il COLORE, dall'alto verso il basso: il turchese della gelatina e' pieno sulla fronte, sul naso e sugli zigomi, dove la luce colpisce piu' diretta; scendendo lungo le guance sfuma, e su labbra, mento e collo la pelle mostra il suo incarnato caldo naturale, ben illuminato. Il passaggio dal turchese al caldo e' graduale, all'altezza delle narici, come il colore di una luce che cala sul viso dall'alto. Il viso si scurisce solo dove la testa gira e smette di guardare la luce: tempie, orecchie e i lati della mascella scendono nell'ombra e da li' nel fondo scuro, cosi' il volto emerge dal buio. Una sola luce: niente luce laterale, niente luce di contorno.
...
await withProject(PROGETTO, async () => {
  ...
  const varianti = [
    { nome: "asse-mento-caldo", prompt: PROMPT.replace(/ciano-verde/g, "ciano").replace(/verde-acqua\/ciano/g, "ciano").replace(/verde-ciano/g, "ciano") },
  ];
  for (let g = 1; g <= giri; g++) {
    ...
    const job = enqueueJob(
      ...
      JSON.stringify({
        recipe: `luce-${v.nome}`,
        materia: MATERIA,
        cambiato: "come v201 (asse-turchese), ma il turchese e' pieno su fronte, naso e zigomi e sfuma verso il basso: labbra, mento e collo restano caldi",
