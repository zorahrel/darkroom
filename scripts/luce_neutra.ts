...
await withProject(PROGETTO, async () => {
  ...
  const varianti = [
    { nome: "neutra-rim-blu", prompt: PROMPT },
  ];
  for (let g = 1; g <= giri; g++) {
    ...
    const job = enqueueJob(
      ...
      JSON.stringify({
        recipe: `luce-${v.nome}`,
        materia: MATERIA,
        cambiato: "come v201 (posa, felpa v4), ma la luce e' quella che claude -p legge nella reference: key BIANCA calda alta-frontale, pelle pesca naturale, blu solo come rim da dietro. Da v168 inseguivo un turchese sul viso che nella reference non c'e'",
