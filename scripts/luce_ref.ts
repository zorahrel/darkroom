...
4. un ritratto di un'ALTRA PERSONA: serve SOLO come campione di luce e di colore. Da questa prendi lo schema di luce, la direzione, la dominante ciano sulla pelle, il fondale blu e il contrasto. Non prenderne il viso, la posa, i capelli, i vestiti ne' l'espressione: il viso resta il mio.
...
VESTITO: indosso la felpa della terza immagine allegata, identica: felpa tutta nera a mezza zip, collo alto a lupetto, zip nera opaca tono su tono chiusa fino a poco sotto il collo, polsini a costine. Sul petto c'e' il piccolo simbolo ricamato tono su tono: DRITTO, orizzontale, centrato sotto la zip, quasi invisibile. Mi sta slim, segue le spalle strette: si vede che sotto sono magro.
...
await withProject(PROGETTO, async () => {
  ...
  const refs = [
    ...
    join(D, "refs", "giacca-armonia-nera-v3.png"),
  ...
  for (let g = 1; g <= giri; g++) {
    ...
    const job = enqueueJob(
      ...
      JSON.stringify(refs),
      JSON.stringify({
        recipe: `luce-ref-campione`,
        materia: MATERIA,
        cambiato: "reference allegata solo come campione di luce e colore; felpa v3 con simbolo dritto",
