...
LA LUCE sul mio viso e' UNA sola, CIANO-TURCHESE chiaro, il colore dell'acqua di una piscina illuminata di notte: un grande pannello morbido quasi frontale, appena piu' alto degli occhi, molto vicino e molto potente. E' l'unica luce che mi tocca il viso, e lo illumina in modo uniforme dalla fronte al mento. Il viso e' SOVRAESPOSTO di mezzo stop, luminoso, quasi abbagliato: fronte, naso, zigomi e mento sono un turchese pallidissimo che sfiora il bianco, le guance un turchese chiaro pieno, e la luce cala morbida verso le zone in ombra. Il turchese sta SOLO dove la luce colpisce la pelle: nelle ombre (sotto gli zigomi, ai lati del naso, sotto il mento, sul collo) la mia pelle mantiene il suo incarnato naturale, caldo e rosato, come succede sotto una vera luce colorata. La pelle resta pelle, con la sua grana e il suo volume sotto la luce colorata. Il viso e' di gran lunga la cosa piu' luminosa della foto; la felpa resta nera.
...
await withProject(PROGETTO, async () => {
  ...
  const varianti = [
    { nome: "ciano-split", prompt: PROMPT.replace(/ciano-verde/g, "ciano").replace(/verde-acqua\/ciano/g, "ciano").replace(/verde-ciano/g, "ciano") },
  ];
