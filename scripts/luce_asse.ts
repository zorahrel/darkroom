...
await withProject(PROGETTO, async () => {
  ...
  const varianti = [
    { nome: "asse-obiettivo", prompt: PROMPT.replace(/ciano-verde/g, "ciano").replace(/verde-acqua\/ciano/g, "ciano").replace(/verde-ciano/g, "ciano") },
  ];
  for (let g = 1; g <= giri; g++) {
    ...
    const job = enqueueJob(
      ...
      JSON.stringify({
        recipe: `luce-${v.nome}`,
        materia: MATERIA,
        cambiato: "luce: da spot vicino al centro del viso a un grande octabox ciano sull'asse dell'obiettivo, stessa luce su tutto il fronte del volto; posa e colore come v197",
        refs: refs.map((r) => r.split("/").pop()),
        misura: "guance bilanciate: min/max guancia sx-dx > 0,8 (v197 0,60); fronte e guance entrambe luminose; bordi e collo in ombra",
        giro: g,
      }),
      "openbrowser",
    );
