...
VESTITO: indosso la felpa della terza immagine allegata, identica: felpa nera a mezza zip, collo alto a lupetto, zip argento chiusa fino a poco sotto il collo, polsini a costine, e il piccolo simbolo a spirale ricamato in bianco sul petto, subito sotto la fine della zip. Mi sta slim, segue le spalle strette: si vede che sotto sono magro.
...
await withProject(PROGETTO, async () => {
  ...
  const refs = [
    ...
    join(D, "refs", "giacca-armonia-nera.png"),
  ];
  ...
  const varianti = [
    { nome: "giacca", prompt: PROMPT.replace(/ciano-verde/g, "ciano").replace(/verde-acqua\/ciano/g, "ciano").replace(/verde-ciano/g, "ciano") },
  ];
  for (let g = 1; g <= giri; g++) {
    ...
    const job = enqueueJob(
      ...
      JSON.stringify({
        ...
        materia: MATERIA,
        cambiato: "ricetta di v179 (tre luci, senza verde, key da destra) con la felpa nera Armonia allegata come accessorio al posto della maglia girocollo",
