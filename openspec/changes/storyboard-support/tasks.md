...
- [ ] 1.1 `tests/` con `bun:test` — il repo non aveva alcun test
- [ ] 1.2 Test: idempotenza migrazioni `db.ts` su DB esistente (riapertura non duplica)
- [ ] 1.3 Test: `assemblePrompt`/`mergeConfig` (promptConfig.ts)
- [ ] 1.4 Test: `sanitizeSteps`/`normalizeGrade` (grade, whitelist step)
- [ ] 1.5 Script `test` in package.json + step `bun test` in `.github/workflows/ci.yml`

## Phase 2 — Schema (server/db.ts, pattern hasColumn esistente)
- [ ] 2.1 `photos.sequence_index INTEGER`
- [ ] 2.2 `photos.duration_ms INTEGER NOT NULL DEFAULT 3000`
- [ ] 2.3 `photos.scene_label TEXT`
- [ ] 2.4 `photos.character_ids TEXT` (JSON array)
- [ ] 2.5 `CREATE TABLE characters (id, name, reference_photo_id, description, created_at)`
- [ ] 2.6 Indici mancanti (esito 0.2): `photos(sequence_index)`, più quelli che il profiling
  delle query di galleria mostra necessari
- [ ] 2.7 `PhotoRow` aggiornato + test di idempotenza verde su copia del DB Japan

## Phase 3 — Backend storyboard
- [ ] 3.1 `server/storyboard.ts`: sequence bulk, CRUD `characters`, creazione pannelli da
  beat sheet (riusa il percorso `generate` esistente, niente motore nuovo)
- [ ] 3.2 `server/storyboardExport.ts`: `exportStoryboard(pid)` → `.storyboarder` + `images/`
  sotto `dirsFor(pid).FINAL_DIR/storyboard/`, involucro confermato in 0.1
- [ ] 3.3 Risoluzione immagine per pannello: favorite version (job `done`) → ultima `done` →
  `original_path`
- [ ] 3.4 Route REST `/api/storyboard/*`
- [ ] 3.5 Test: progetto fixture 3 pannelli → JSON valido, `time` cumulativo corretto,
  3 immagini copiate

## Phase 4 — Split server/index.ts (1371 righe → moduli route)
- [ ] 4.1 `server/routes/*.ts` montati su Hono, `index.ts` ridotto a bootstrap+middleware
- [ ] 4.2 Refactor a comportamento invariato: test di Phase 1/3 verdi prima e dopo

## Phase 5 — Personaggi nella generazione (sbloccata da 0.3)
- [ ] 5.1 `upload_file` accetta N path (un solo `DOM.setFileInputFiles`), attesa di N thumb
- [ ] 5.2 `jobs.ref_paths` (JSON) propagata dalla API al worker
- [ ] 5.3 Prompt che nomina i personaggi allegati (coerenza cross-panel)
- [ ] 5.4 Fallback: se l'attesa thumb fallisce, genera col solo input primario (mai bloccare)

## Phase 6 — Client
- [ ] 6.1 Route `/storyboard` lazy-loaded: pannelli riordinabili, durata, etichetta scena
- [ ] 6.2 Pannello personaggi (crea da foto, pinna sul board)
- [ ] 6.3 Generazione pannelli da beat sheet + bottone Export
- [ ] 6.4 Split `Detail.tsx` (1196) e `api.ts` (727) senza cambi funzionali

## Phase 7 — MCP (mcp/server.ts)
- [ ] 7.1 `set_sequence({ids})`
- [ ] 7.2 `export_storyboard({})`
- [ ] 7.3 `create_panels({beats})`
- [ ] 7.4 `list_characters` / `set_character`

## Phase 8 — Verifica end-to-end
- [ ] 8.1 typecheck + `bun test` + build client verdi
- [ ] 8.2 App avviata: galleria Japan intatta (190 foto), editor e grade funzionanti
- [ ] 8.3 Flusso storyboard reale fino al file `.storyboarder` esportato
- [ ] 8.4 Conteggi DB confrontati pre/post (photos 190, versions 1647, jobs 104, orphans 130)
- [ ] 8.5 README/CONTRIBUTING aggiornati
