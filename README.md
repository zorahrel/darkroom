...
- **Collage slides** — merge the weaker shots of a post into a single carousel slide: pick 2-9 photos inside one post and compose them as *hero* (one big, the rest in a strip), *mosaic*, *grid*, *stacked* or *split*. Every mode is full-bleed — no frames, no visible background — and the rendered JPG is the same file the export writes. Dissolving a collage puts the photos back exactly where they were.
