...
- **Post harmonisation** — every AI render comes back with its own colour cast, so a carousel of the same afternoon drifts from frame to frame. A `match` grade step aligns each photo to the median of *its post* in Lab space — chroma only, luminance untouched, so an interior stays dark and a sunlit shot stays bright. Measured on this set: chroma spread halved (25.4 → 11.6) with luminance spread unchanged. This is Photoshop's Match Color, applied to a group instead of a pair.
