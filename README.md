...
- **Collections (posts)** — group the gallery into the units you actually publish: each collection is an ordered subset (a post or carousel) with a title and a caption. Group the grid by "Post", assign a selection in bulk, rename or dissolve a post from its header, drag tiles to set the running order (slide 1 is the cover, numbered in place), and filter by "Non assegnate" to work through what is not published yet. Membership is exclusive, so a photo can't silently go out twice.
- **Collage slides** (off by default) — merge photos of a post into one carousel slide: *hero*, *mosaic*, *grid*, *stacked* or *split*, all full-bleed. Only earns its keep when the photos are the same subject repeated; enable with `localStorage.darkroom.collage = "1"`.
- **Post colour reference** — every AI render picks its own colour in isolation, so a carousel shot in one afternoon drifts frame to frame. Rather than filtering it back into line afterwards, each post nominates a reference photo: it is attached to every generation in that group as a second image, with an explicit "match its colour treatment, never its subject" clause. The coherent colour is *rendered*, not corrected. A deterministic `match` grade step (Lab chroma transfer, luminance untouched) remains available for sets that cannot be regenerated.
